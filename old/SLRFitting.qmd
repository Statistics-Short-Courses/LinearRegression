...
Residuals, $e$, are similar to the errors, $\varepsilon$, that we encountered in @sec-introduction-to-linear-models - but they are distinct. In many situations, we do not know the 'acutual' values of $\alpha$ and $\beta$ - so we cannot calculate the errors, $\varepsilon= Y- E[Y]= Y-\alpha + \beta X$. However, we do  have our estimates, $a$ and $b$, so we can calculate residuals. 
