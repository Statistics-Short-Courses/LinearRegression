...
```{webr chapter01-slr-setup}
#| include: false
library(tidyverse)
theme_set(theme_bw())
```

```{r chapter01-slr-setup-2}
#| include: false
library(tidyverse)
theme_set(theme_bw())
```

```{r chapter01-slr-setup-simulate}
#| echo: false
#| define:
#|  - linearData
n <- 20
...
```{r chapter01-slr-fitting-models-to-data-plot}
#| echo: false
#| input:
#|   - linearData
ggplot(linearData)+
...
```{r chapter01-slr-fitting-models-to-data}
#| echo: false
#| input:
#|   - linearData
linearData
...
```{webr chapter01-slr-fitting-a-model-with-the-lm-function-fit-lm}
#| input:
#|   - linearData
...
```{webr chapter01-slr-fitting-a-model-with-the-lm-function}
lm_1$coefficients
```
or by the `coef()` function:

```{webr chapter01-slr-fitting-a-model-with-the-lm-function-2}
coef(lm_1)
```
...
```{webr chapter01-slr-estimating-variance}
residuals(lm_1)
```
...
```{webr chapter01-slr-seb-pvalue-visual}
#| echo: false
#| input:
#|   - linearData
lm_1 <- lm(y ~ x, linearData)
...
```{webr chapter01-slr-seb-ci-visual}
#| echo: false
#| input:
#|   - linearData
lm_1 <- lm(y ~ x, linearData)
...
```{webr chapter01-slr-confidence-interval-for-the-slope}
confint(lm_1, "x")
```
