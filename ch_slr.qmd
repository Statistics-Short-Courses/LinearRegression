...
```{r ch-slr-fitting-models-to-data-plot}
#| echo: false
#| input:
#|   - linearData
ggplot(linearData)+
...
```{r ch-slr-fitting-models-to-data}
#| echo: false
#| input:
#|   - linearData
linearData
...
```{webr ch-slr-fitting-a-model-with-the-lm-function-fit-lm}
#| input:
#|   - linearData
...
```{webr ch-slr-fitting-a-model-with-the-lm-function}
lm_1$coefficients
```
or by the `coef()` function:

```{webr ch-slr-fitting-a-model-with-the-lm-function-2}
coef(lm_1)
```
...
```{webr ch-slr-estimating-variance}
residuals(lm_1)
```
...
```{webr ch-slr-confidence-interval-for-the-slope}
confint(lm_1, "x")
```
