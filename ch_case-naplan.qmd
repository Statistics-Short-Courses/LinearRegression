...
## Diagnostics (does the model look reasonable?) {#sec-case-naplan-diagnostics}

As emphasized in @sec-regression-pitfalls-and-diagnostics, we always re-check residual diagnostics on the final model (see @sec-common-diagnostic-plots and @sec-diagnosing-issues-and-choosing-remedies).

### A small set of diagnostic plots

```{webr ch-case-naplan-diagnostics-plots}
#| echo: true
#| autorun: true
...
The residual-vs-fitted plot is the main place to look for nonlinearity and changing variance (see @sec-diagnosing-nonlinearity and @sec-diagnosing-heteroskedasticity). In this case the pattern looks broadly centred around 0, with at most mild changes in spread.

### Influence and unusual points

```{webr ch-case-naplan-diagnostics-influence}
#| echo: true
#| autorun: true
final_aug |>
  arrange(desc(.cooksd)) |>
...
::: Exercise
Recreate the core diagnostic plot from @sec-common-diagnostic-plots: residuals vs fitted values.
...
grade_this({
  ...
  pass("Nice — residual plots are a core diagnostic tool (see @sec-common-diagnostic-plots).")
})
```
:::
:::
