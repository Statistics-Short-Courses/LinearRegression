...
We now fit regression models using [ordinary least squares]{.glossary term="Ordinary least squares"} (see @sec-a-simple-linear-model-in-r). We start with a **first-order** additive model and then consider targeted increases in complexity when the EDA suggests they are useful (see @sec-increasing-model-complexity-to-model-complex-relationships).
