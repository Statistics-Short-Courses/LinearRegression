...
```{webr ch-mlr-advertising-sales-load-data}
#| edit: false
...
```{webr ch-mlr-coloured-scatter-plot}
#| edit: false
#| echo: false
...
```{webr ch-mlr-multivariate-linearity-fit-lm}
#| echo: false
#| edit: false
...
```{r ch-mlr-interpreting-a-partial-coefficient-beta-i-plot}
ggplot()+
...
```{webr ch-mlr-fitting-mlr-models-in-r-fit-lm}
...
- this will be adressed more comprehensively in @sec-multicollinearity-diagnostics.
:::
...
```{webr ch-mlr-global-statistics-with-anova}
anova(mod)
```
...
```{webr ch-mlr-global-statistics-with-glance}
glance(mod)
```
...
Because AIC and BIC are tools for *model selection* rather than model assessment, we do not interpret them here. They will be discussed in detail in @sec-information-criteria-aic-bic and @sec-comparing-model-fit, where they are used to guide decisions about which predictors to include in a regression model.
...
```{webr ch-mlr-transmission-means}
#| echo: true
mtcars_cat %>%
  group_by(am_f) %>%
