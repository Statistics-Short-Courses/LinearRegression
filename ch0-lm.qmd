...
```{r chapter00-lm-setup}
#| include: false
library(ggplot2)
theme_set(theme_bw())
```

```{webr chapter00-lm-setup-2}
#| include: false
library(ggplot2)
theme_set(theme_bw())
```
...
This allows us to describe the **[expected value]{.glossary term="Expected value"}** of $Y$ as
...
```{ojs chapter00-lm-linear-prediction-view}
//| panel: sidebar
//| echo: false
...
```{ojs chapter00-lm-linear-prediction-ojs}
//| panel: fill
//| echo: false
xRange = [-10,10]
...
```{r chapter00-lm-salary-growth-over-time-plot}
#| echo: false
...
```{r chapter00-lm-salary-growth-over-time-code}
#| eval: false
ggplot() +
  geom_abline(intercept = 5e4, slope = 5e3, colour = '#2196F3') +
  lims(x = c(0, 15), y = c(4e4, 13e4)) +
...
```{webr chapter00-lm-choosing-parameters}
#| exercise: ex_1.1.1
#| envir: Ex1
alpha <- _______
beta <- _______
...
```{webr chapter00-lm-choosing-parameters-solution}
#| exercise: ex_1.1.1
#| solution: true
#| envir: Ex1
alpha <- 70000
...
```{webr chapter00-lm-choosing-parameters-solution-2}
#| exercise: ex_1.1.1
#| check: true
#| class: wait
#| envir: Ex1
...
```{r chapter00-lm-linear-prediction-plot}
#| echo: false
...
```{r chapter00-lm-linear-prediction-code}
#| eval: false
ggplot() +
  geom_abline(aes(intercept = 7e4, slope = 3e3, colour = "Company B")) +
  geom_abline(aes(intercept = 5e4, slope = 5e3, colour = "Company A")) +
...
```{webr chapter00-lm-linear-prediction-code-2}
#| exercise: ex_1.1.2
#| envir: Ex1
...
```{webr chapter00-lm-linear-prediction-code-3}
#| exercise: ex_1.1.2
#| solution: true
#| envir: Ex1
E_Y <- alpha + (beta * 15)
...
```{webr chapter00-lm-linear-prediction-code-4}
#| exercise: ex_1.1.2
#| check: true
#| class: wait
#| envir: Ex1
...
```{webr chapter00-lm-using-r-functions}
#| exercise: ex_1.1.2.2
#| envir: Ex1
#| edit: false
E_Y
...
```{webr chapter00-lm-using-r-functions-2}
#| edit: false
simple_linear_prediction <- function(x, alpha = 7e4, beta = 3e3) {
  alpha + (beta * x)
}
```
...
```{webr chapter00-lm-using-r-functions-3}
#| edit: false
x <- c(9, 4, 5, 2, 6)
```

```{webr chapter00-lm-using-r-functions-4}
#| exercise: ex_1.1.3
#| envir: Ex1
...
```{webr chapter00-lm-using-r-functions-5}
#| exercise: ex_1.1.3
#| solution: true
...
```{webr chapter00-lm-using-r-functions-6}
#| exercise: ex_1.1.3
#| check: true
#| class: wait
gradethis::grade_this_code()
...
While the assumptions of $\mu=0$ and $Var(\varepsilon)=\sigma^2$ describe the center and spread of the errors, they don’t fully specify the shape of their distribution. If we want to draw precise inferences about probability events (e.g. to test hypotheses using p-values), we can make a stronger assumption about the [distribution]{.glossary} of the errors. In particular, we choose to assume that the errors follow a [Normal distribution]{.glossary}.
...
```{ojs chapter00-lm-normal-distribution-view}
//| echo: false
//| panel: sidebar
...
```{ojs chapter00-lm-normal-distribution-ojs}
//| echo: false
//| panel: fill
...
```{r chapter00-lm-variation-in-salary}
50000+(5000*10)
```
...
```{r chapter00-lm-variation-in-salary-2}
...
```{ojs chapter00-lm-variation-in-salary-view}
//| echo: false
//| panel: sidebar
...
```{ojs chapter00-lm-variation-in-salary-ojs}
//| echo: false
//| panel: fill
...
:::: {}
```{webr chapter00-lm-normally-distributed-errors}
#| exercise: ex_1.2.1
#| envir: Ex2
...
```{webr chapter00-lm-normally-distributed-errors-solution}
#| exercise: ex_1.2.1
#| solution: true
#| envir: Ex2
...
```{webr chapter00-lm-normally-distributed-errors-solution-2}
#| exercise: ex_1.2.1
#| check: true
#| class: wait
#| envir: Ex2
...
```{ojs chapter00-lm-the-simple-linear-model-view}
//| panel: sidebar
//| echo: false
...
```{ojs chapter00-lm-the-simple-linear-model-ojs}
//| panel: fill
//| echo: false
//| width: 700
...
```{r chapter00-lm-a-simple-linear-model-in-r-simulate}
n <- 100
...
```{r chapter00-lm-a-simple-linear-model-in-r-simulate-2}
...
```{r chapter00-lm-a-simple-linear-model-in-r}
...
```{r chapter00-lm-a-simple-linear-model-in-r-setup}
#| echo: false
...
```{r chapter00-lm-a-simple-linear-model-in-r-code}
#| eval: false
library(ggplot2)
...
```{webr chapter00-lm-a-simple-linear-model-in-r-code-2}
#| code-fold: true
