...
## [Partial regression coefficients]{.glossary term="Partial regression coefficient"}, $\beta_i$
...
::: Exercise
### Calculating the expected outcome of a multiple regression model
...
```{webr module02-mlr-calculating-the-expected-outcome-of-a-multiple-r}
...
```{webr module02-mlr-extracting-coefficients-from-an-lm-model-plot}
ggplot(ads)+
  aes(x=TV, y=Sales)+
  geom_point()+
...
::: Key-point
#### Interpreting the [t-test]{.glossary term="t-test"} of $\beta_i$
...
::: Note 
### Multiple [t-tests]{.glossary term="t-test"} and type I error inflation
...
## The [F-test]{.glossary term="F-test (overall model test)"} for model model usefulness. :  and measures of model fit
