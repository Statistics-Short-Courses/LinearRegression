...
```{webr chapter03-model-building-setup}
#| include: false
library(tidyverse)
library(plotly)
theme_set(theme_bw())
...
```{r chapter03-model-building-setup-2}
#| include: false
library(tidyverse)
theme_set(theme_bw())
```
...
# Model Building and Variable Selection {#sec-model-building-and-selection}

In this chapter, we explore strategies for building effective regression models. We cover:
...
```{r chapter03-model-building-non-linear-relationships-in-data-simulate}
#| echo: false
#| define:
#|  - quadData
set.seed(12)
...
```{r chapter03-model-building-third-order-cubic-univariate-model-simulate}
#| echo: false
#| define:
#|  - cubicData
set.seed(21)
...
```{r chapter03-model-building-fitting-higher-order-polynomial-terms-simulate}
#| echo: false
#| define:
#|  - linearData
set.seed(1)
...
```{r chapter03-model-building-fitting-higher-order-univariate-models-in-r-fit-lm}
#| echo: true
lm(y ~ x + I(x^2), data = nonlinearData)
```
or using the `poly()` function:
```{r chapter03-model-building-fitting-higher-order-univariate-models-in-r-fit-lm-2}
#| echo: true
...
```{r chapter03-model-building-fitting-an-interaction-model-to-the-advertising-load-data}
#| echo: true
#| edit: false
...
```{r chapter03-model-building-fitting-an-interaction-model-to-the-advertising-setup}
#| edit: false
#| echo: false
library(plotly)
...
```{r chapter03-model-building-fitting-an-interaction-model-to-the-advertising-fit-lm}
#| echo: false
#| edit: false
...
```{r chapter03-model-building-fitting-an-interaction-model-to-the-advertising-predict}
#| edit: false
#| echo: false
...
```{r chapter03-model-building-fitting-an-interaction-model-to-the-advertising-setup-2}
#| echo: false
library(ggplot2)
library(scales)
...
However, as before, increasing model complexity in this way raises the risk of [overfitting]{.glossary} and interpretability challenges, which we will discuss later in this chapter.
...
```{r chapter03-model-building-fitting-interaction-models-in-r-fit-lm}
#| echo: true
#| eval: false
...
```{r chapter03-model-building-fitting-interaction-models-in-r-fit-lm-2}
#| echo: true
#| eval: false
...
```{webr chapter03-model-building-cat-interaction-setup}
#| include: false
mtcars_cat <- mtcars %>%
  mutate(
...
```{webr chapter03-model-building-continuous-by-categorical-fit-lm}
#| echo: true
mod_am_wt <- lm(mpg ~ wt * am_f, data = mtcars_cat)
summary(mod_am_wt)$coefficients
```

```{webr chapter03-model-building-continuous-by-categorical-plot}
#| echo: false
...
```{webr chapter03-model-building-categorical-by-categorical-fit-lm}
#| echo: true
...
```{r chapter03-model-building-calculating-fit-metrics-with-broom-glance-setup}
#| echo: true
library(broom)
glance(ads_mod_int)
```
...
```{r chapter03-model-building-table-of-fit-metrics}
...
```{r chapter03-model-building-table-of-fit-metrics-code}
#| echo: true
#| eval: false
ads_fit_glance <- rbind(
  glance(ads_mod) |> `rownames<-`("First-order model"),
...
```{r chapter03-model-building-table-of-fit-metrics-code-2}
#| echo: true
...
```{r chapter03-model-building-pairs-plots-and-correlation-matrices-setup}
#| echo: true
#| eval: false
library(GGally)
ggpairs(ads)
...
```{r chapter03-model-building-samara-comparing-seeds-across-trees-load-data}
#| echo: true
samara <- read_csv("Data/samara.csv", show_col_types = FALSE) |>
  mutate(Tree = factor(Tree))
```
```{r chapter03-model-building-samara-comparing-seeds-across-trees-plot}
...
**Candidate models**
```{r chapter03-model-building-samara-comparing-seeds-across-trees-fit-lm}
#| echo: true 
sam_mods <- list(
...
```{r chapter03-model-building-samara-comparing-seeds-across-trees-anova}
#| echo: true
...
```{r chapter03-model-building-samara-comparing-seeds-across-trees-fit-lm-2}
#| echo: true
