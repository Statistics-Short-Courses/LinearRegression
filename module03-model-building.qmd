...
The interaction effect can also be visualised by fixing one predictor and plotting the relationship between the other predictor and the outcome. The slope of this relationship changes depending on the fixed value, illustrating the interaction.
```{r}
...
```
This expands to include both main effects and the interaction term. To include only the interaction term without main effects, use the `:` operator:
```{r}
#| echo: true 
#| edit: false
lm(Y ~ X1:X2, data = mydata)
