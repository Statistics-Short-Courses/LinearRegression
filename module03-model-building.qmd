...
```{webr module03-model-building-setup}
#| include: false
library(tidyverse)
library(plotly)
theme_set(theme_bw())
...
```{r module03-model-building-setup-2}
#| include: false
library(tidyverse)
theme_set(theme_bw())
```
...
# Model Building and Selection {#sec-model-building-and-selection}
...
- Techniques for selecting [parsimonious]{.glossary term="Parsimony"} models.

## Increasing model complexity to model complex relationships {#sec-increasing-model-complexity-to-model-complex-relationships}

### Why increase model complexity? {#sec-why-increase-model-complexity}
...
::: Key-term
#### First-order model {#sec-first-order-model}
...
```{r module03-model-building-non-linear-relationships-in-data-simulate}
#| echo: false
#| define:
#|  - quadData
set.seed(12)
...
```{ojs module03-model-building-non-linear-relationships-in-data-view}
//| panel: sidebar
//| echo: false
...
```{ojs module03-model-building-non-linear-relationships-in-data-ojs}
//| panel: fill
//| echo: false
...
### Square, Cubic, and Higher-Order Univariate Models {#sec-non_linearity}
...
::: Key-term
#### Second-order (Quadratic) univariate model {#sec-second-order-quadratic-univariate-model}
  $$E[Y] = \beta_0 + \beta_1X + \beta_2X^2$$
:::
::: Example
#### Seeing how $\beta_2$ bends the curve {#sec-seeing-how-beta2-bends-the-curve}
...
```{ojs module03-model-building-seeing-how-beta-2-bends-the-curve-view}
//| panel: sidebar
//| echo: false
...
```{ojs module03-model-building-seeing-how-beta-2-bends-the-curve-ojs}
//| panel: fill
//| echo: false
...
#### Third-order (cubic) univariate model {#sec-third-order-cubic-univariate-model}
$$
...
```{r module03-model-building-third-order-cubic-univariate-model-simulate}
#| echo: false
#| define:
#|  - cubicData
set.seed(21)
...
::: Example
#### Seeing how $\beta_3$ twists the curve {#sec-seeing-how-beta3-twists-the-curve}
...
```{ojs module03-model-building-seeing-how-beta-3-twists-the-curve-view}
//| panel: sidebar
//| echo: false
...
```{ojs module03-model-building-seeing-how-beta-3-twists-the-curve-ojs}
//| panel: fill
//| echo: false
...
#### N-th order univariate model {#sec-ho_univariate}
...
::: Example
#### Fitting higher-order [polynomial terms] {#sec-fitting-higher-order-polynomial-terms .glossary term="Polynomial term"}
```{r module03-model-building-fitting-higher-order-polynomial-terms-simulate}
#| echo: false
#| define:
#|  - linearData
set.seed(1)
...
```{ojs module03-model-building-fitting-higher-order-polynomial-terms-view}
//| echo: false
...
```{ojs module03-model-building-fitting-higher-order-polynomial-terms-ojs}
//| echo: false
...
```{ojs module03-model-building-fitting-higher-order-polynomial-terms-ojs-2}
...
::: Note
#### The meaning of "linear" in linear models {#sec-the-meaning-of-linear-in-linear-models}
...
#### Fitting higher-order univariate models in R {#sec-fitting-higher-order-univariate-models-in-r}
...
```{r module03-model-building-fitting-higher-order-univariate-models-in-r-fit-lm}
#| echo: true
lm(y ~ x + I(x^2), data = nonlinearData)
```
or using the `poly()` function:
```{r module03-model-building-fitting-higher-order-univariate-models-in-r-fit-lm-2}
#| echo: true
...
### Interaction Models with Continuous Predictors {#sec-interaction-models-with-continuous-predictors}
...
Now however, we can also consider a different form of higher order term: what happens when we multiply predictors together? This gives us an **[interaction term]{.glossary term="Interaction"}** - a term that combines two (or more) predictors.

::: Key-term
#### Interaction term {#sec-interaction-term}
...
::: Example
#### Fitting an interaction model to the `advertising` dataset. {#sec-fitting-an-interaction-model-to-the-advertising-dataset}
If we recall our plot of the `advertising` dataset from @sec-multiple_linear_regression, our fist-order linear model did not fit the data in particular ways:
...
```{r module03-model-building-fitting-an-interaction-model-to-the-advertising-load-data}
#| echo: true
#| edit: false
...
```{r module03-model-building-fitting-an-interaction-model-to-the-advertising-setup}
#| edit: false
#| echo: false
library(plotly)
...
```{r module03-model-building-fitting-an-interaction-model-to-the-advertising-fit-lm}
#| echo: false
#| edit: false
...
```{r module03-model-building-fitting-an-interaction-model-to-the-advertising-predict}
#| edit: false
#| echo: false
...
```{r module03-model-building-fitting-an-interaction-model-to-the-advertising-setup-2}
#| echo: false
library(ggplot2)
library(scales)
...
#### Higher-order interaction models {#sec-higher-order-interaction-models}
...
However, as before, increasing model complexity in this way raises the risk of [overfitting]{.glossary} and interpretability challenges, which we will discuss later in this module.
...
#### Fitting interaction models in R {#sec-fitting-interaction-models-in-r}
...
```{r module03-model-building-fitting-interaction-models-in-r-fit-lm}
#| echo: true
#| eval: false
...
```{r module03-model-building-fitting-interaction-models-in-r-fit-lm-2}
#| echo: true
#| eval: false
...
### Interaction Models with Categorical Predictors {#sec-mlr-cat-interactions}

As in @sec-mlr_cat, a categorical predictor with $k$ levels is represented by $k-1$ dummy variables:
$$
...
```{webr module03-model-building-cat-interaction-setup}
#| include: false
mtcars_cat <- mtcars %>%
  mutate(
...
#### Continuous $\times$ categorical interactions {#sec-continuous-by-categorical-interactions}
...
```{webr module03-model-building-continuous-by-categorical-fit-lm}
#| echo: true
mod_am_wt <- lm(mpg ~ wt * am_f, data = mtcars_cat)
summary(mod_am_wt)$coefficients
```

```{webr module03-model-building-continuous-by-categorical-plot}
#| echo: false
ggplot(mtcars_cat, aes(x = wt, y = mpg, color = am_f)) +
  geom_point() +
...
#### Categorical $\times$ categorical interactions {#sec-categorical-by-categorical-interactions}
...
```{webr module03-model-building-categorical-by-categorical-fit-lm}
#| echo: true
mod_cyl_am <- lm(mpg ~ cyl_f * am_f, data = mtcars_cat)
anova(mod_cyl_am)
```
...
## Assessing model fit and model complexity {#sec-assessing-model-fit-and-model-complexity}

### Why not increase model complexity? {#sec-why-not-increase-model-complexity}

In the previous section we saw how adding higher-order terms and interactions can help us capture complex relationships in data. However, increasing model complexity is not always beneficial. While more complex models can fit the *training data* (i.e. the data we use to fit our model) better, they may not generalise well to new data. This is because complex models can start to fit random noise in the training data, rather than the underlying signal. This phenomenon is known as **[overfitting]{.glossary}**.
...
::: Key-point
#### Balancing fit and complexity {#sec-balancing-fit-and-complexity}
...
::: Key-term
#### Coefficient of Determination ($R^2$) {#sec-coefficient-of-determination-r-2}
$$
...
#### [Adjusted $R^2$] {#sec-adjusted-r-2-overview .glossary term="Adjusted R-squared"}
...
::: Key-term
#### [Adjusted $R^2$] {#sec-adjusted-r-2-definition .glossary term="Adjusted R-squared"}
$$
...
#### Information Criteria: AIC, BIC {#sec-information-criteria-aic-bic}
...
::: Key-term
#### [Akaike Information Criterion (AIC)] {#sec-akaike-information-criterion-aic .glossary term="Akaike Information Criterion"}
$$
\text{AIC} = 2k - 2\ln(L)
$$
...
::: Key-term
#### [Bayesian Information Criterion (BIC)] {#sec-bayesian-information-criterion-bic .glossary term="Bayesian Information Criterion"}
$$
...
::: Key-point
#### Interpreting AIC and BIC {#sec-interpreting-aic-and-bic}
...
#### Calculating fit metrics with `broom::glance()` {#sec-calculating-fit-metrics-with-broom-glance}
...
```{r module03-model-building-calculating-fit-metrics-with-broom-glance-setup}
#| echo: true
library(broom)
glance(ads_mod_int)
```
...
#### Comparing model fit {#sec-comparing-model-fit}
...
::: Example
#### Comparing first-order and interaction models {#sec-comparing-first-order-and-interaction-models}
Lets continue our investigation of the `advertising` dataset from @sec-multiple_linear_regression by comparing our first-order model of Sales (`ads_mod`) to our second-order interaction model (`ads_mod_int`).

Lets combine the fit metrics from both models into a single table for easy comparison:

::: {.panel-tabset}
##### Table of fit metrics {#sec-table-of-fit-metrics}
```{r module03-model-building-table-of-fit-metrics}
...
##### Code {#sec-table-of-fit-metrics-code}
```{r module03-model-building-table-of-fit-metrics-code}
#| echo: true
#| eval: false
ads_fit_glance <- rbind(
  glance(ads_mod) |> `rownames<-`("First-order model"),
...
```{r module03-model-building-table-of-fit-metrics-code-2}
#| echo: true
...
## A model building workflow {#sec-a-model-building-workflow}
...
### Exploratory data analysis and theory-driven model development {#sec-exploratory-data-analysis-and-theory-driven-model-development}
...
#### Pairs plots and correlation matrices {#sec-pairs-plots-and-correlation-matrices}
...
```{r module03-model-building-pairs-plots-and-correlation-matrices-setup}
#| echo: true
#| eval: false
library(GGally)
ggpairs(ads)
```
### Automated model selection methods {#sec-automated-model-selection-methods}

### [Stepwise regression] {#sec-stepwise-regression .glossary}

Stepwise methods provide automated ways to search for simpler models.

#### Forward Selection {#sec-forward-selection}

Begin with a minimal model (often the intercept). Add predictors one at a time when they improve AIC or adjusted $R^2$.

#### Backward Selection {#sec-backward-selection}
...
::: Example
#### Samara: comparing seeds across trees {#sec-samara-comparing-seeds-across-trees}
Maple samara (seeds) fall like tiny helicopters. A forest scientist measured their fall speed (`Velocity`) against their 'disk loading' (a quantity based on their size and weight; encoded as`Load`) on three trees (`Tree`). The goal: check whether the rate of change in velocity differs between trees.
```{r module03-model-building-samara-comparing-seeds-across-trees-load-data}
#| echo: true
samara <- read_csv("Data/samara.csv", show_col_types = FALSE) |>
  mutate(Tree = factor(Tree))
```
```{r module03-model-building-samara-comparing-seeds-across-trees-plot}
ggplot(samara, aes(Load, Velocity, colour = Tree)) +
  geom_point(size = 2, alpha = 0.85, show.legend = FALSE) +
  geom_smooth(method = "lm", se = FALSE, show.legend = FALSE) +
  labs(caption = "Faceting makes it easier to compare slopes across trees.")
```
...
**Candidate models**
```{r module03-model-building-samara-comparing-seeds-across-trees-fit-lm}
#| echo: true 
sam_mods <- list(
...
```{r module03-model-building-samara-comparing-seeds-across-trees-anova}
#| echo: true
...
```{r module03-model-building-samara-comparing-seeds-across-trees-fit-lm-2}
#| echo: true
