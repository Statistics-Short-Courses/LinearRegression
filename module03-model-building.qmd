...
::: Example
#### Samara: comparing slopes across trees
Maple samara fall like tiny helicopters. A forest scientist measured their fall speed (`Velocity`) against their 'disk loading' (a quantity based on their size and weight; encoded as`Load`) on three trees (`Tree`). The goal: check whether the rate of change in velocity differs between trees.
```{r}
#| echo: true
