...
![Linear Regression with R](linear-regression-with-r-banner-v3.svg){.course-banner}
...
- @sec-introduction-to-linear-models: what a regression model is, why we need an error term, and what assumptions mean in practice.
- @sec-slr: least squares, fitted values and residuals, inference, and prediction.
- @sec-multiple-linear-regression-mlr: partial regression coefficients, categorical predictors, and overall model tests.
- @sec-model-building-and-selection: interactions, polynomial terms, parsimony, and model comparison/selection.
- @sec-regression-pitfalls-and-diagnostics: how to recognise assumption violations and influence, and what to do about them.
- @sec-regression-case-study-naplan-reading-scores: a guided workflow through a real dataset from research question → model → diagnostics → interpretation.
...
- Basic `R` usage, including famiarity with the `tidyverse` including `ggplot2` and `dplyr` (see the [UNE Consolidated R Resources](https://turing.une.edu.au/~statshare/index.html) for a refresher)
