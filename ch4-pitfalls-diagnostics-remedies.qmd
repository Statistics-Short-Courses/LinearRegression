...
In @sec-model-building-and-selection we saw that complex regression models can go wrong due to [overfitting]{.glossary term="Overfitting"}. But even regression analyses can be misleading if the linear model we are fitting does not adequately capture the data-generating process.
...
```{r chapter04-regression-pitfalls-diagnostics-anscombe-summary}
#| edit: false
anscombe_long |>
  group_by(set) |>
  summarise(
...
```
 ...
 ::: {.panel-tabset}
 ### fitted lines
```{r chapter04-regression-pitfalls-diagnostics-anscombe-fitted-lines}
#| edit: false
#| include: false
#| echo: false
...
ggplot(anscombe_long, aes(x = x, y = y)) +
  ...
  facet_wrap(~ set, ncol = 2) 
```
```{r chapter04-regression-pitfalls-diagnostics-anscombe-regression}
#| edit: false
list(
